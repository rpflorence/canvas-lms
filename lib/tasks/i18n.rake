    ...
    files.each do |file|
      ...
      locales.each do |locale|
        translations.each do |key|
          scope = key.split('.')[0]
          translated_scopes[scope] ||= {}
          translated_scopes[scope]["#{locale}.#{key}"] = all_translations["#{locale}.#{key}"] if all_translations["#{locale}.#{key}"]
        end
      end
    end
...
javascripts:
    ...
    scope_it = proc { |scope, *args|
      translations = args.shift
      translation_name = args.shift || scope
      scope_file = "public/javascripts/translations/#{translation_name}.js"
...
define(['i18n', 'jquery'], function(I18n, $) {
  $.extend(true, I18n, {translations: #{translations.to_json}});
      ...
      TRANSLATIONS
      if !File.exist?(scope_file) || File.read(scope_file) != content
        File.open(scope_file, "w"){ |f| f.write content }
      end
    }

    all_translations = I18n.backend.send(:translations)
    translated_scopes.each do |scope, translations|
      scope_it.call(scope, translations.expand) unless translations.empty?
    end
    exit
    ...
    scope_it.call(:common, english_core_translations, '_core_en')
    scope_it.call(:common, core_translations, '_core')
